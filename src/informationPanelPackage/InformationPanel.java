...
public class InformationPanel extends JPanel
{
	...
	public void addNewMoveDescription(String explicitMoveDescription,String standardMoveDescription,Boolean isSpecial)
	{
	...
	public String GetPieceTypeEventuallyDeleted(int indexMoves)
	{
		...
		if(indexEat!=-1)
		{
			if(listMovesDescriptionExplicit.get(indexMoves).indexOf(checkDescription)!=-1)
				return listMovesDescriptionExplicit.get(indexMoves).substring(indexEat+eatString.length()+1,
				listMovesDescriptionExplicit.get(indexMoves).substring(indexEat+eatString.length()+1).indexOf(" ")+indexEat+eatString.length()+1);
			else
				return listMovesDescriptionExplicit.get(indexMoves).substring(indexEat+eatString.length()+1,
				listMovesDescriptionExplicit.get(indexMoves).length());
		}
		else
			return "";
	}
